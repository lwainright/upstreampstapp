...
// ── Agency codes (demo data) ───────────────────────────────────────────────────
const AGENCY_CODES = {
  UPSTREAM:  {name:"Upstream Demo",adminEmail:"admin@upstream.com",contractExpiry:null,codeExpiry:null},
  DEMO123:   {name:"Demo Agency",  adminEmail:"demo@demo.com",     contractExpiry:null,codeExpiry:null},
  PST123:    {name:"PST Demo",     adminEmail:"pst@demo.com",      contractExpiry:null,codeExpiry:null},
};
...
const getDaysUntilPurge = (agData) => {
  if(!agData||!agData.contractExpiry) return 180;
  const diff = new Date(agData.contractExpiry) - new Date();
  const daysSince = Math.floor(-diff / (1000*60*60*24));
  return Math.max(0, 180 - daysSince);
};
 
// ── fetchAgencyStats ───────────────────────────────────────────────────────────
const AW_ENDPOINT_VAL = import.meta.env.VITE_APPWRITE_ENDPOINT || 'https://nyc.cloud.appwrite.io/v1';
const AW_PROJECT_VAL  = import.meta.env.VITE_APPWRITE_PROJECT  || 'upstreamapproach';
const AW_DB_VAL       = import.meta.env.VITE_APPWRITE_DATABASE || 'upstream_db';
...
async function fetchAgencyStats(agencyCode, days=30) {
  try {
    ...
    const headers = { 'X-Appwrite-Project': AW_PROJECT_VAL };
    const base = `${AW_ENDPOINT_VAL}/databases/${AW_DB_VAL}/collections`;
...
export default function AdminToolsScreen({navigate,membership,onSwitchAgency,pstAlert,setPstAlert,pstAlertMsg,setPstAlertMsg,criticalIncident,setCriticalIncident,setAgencyNotification,isPlatform=false,onGhostLogin}){
  ...
  const[pstRoster,setPstRoster]=useState(DEMO_PST_ROSTERS[agencyKey]||DEMO_PST_ROSTERS["UPSTREAM"]);
  const[escalations,setEscalations]=useState(DEMO_ESCALATIONS[agencyKey]||[]);
    ...
    <ScreenSingle headerProps={hp}>
      ...
      {tab==="overview"&&(
        <div>
          ...
          {statsError&&<div style={{background:"rgba(234,179,8,0.07)",border:"1px solid rgba(234,179,8,0.18)",borderRadius:10,padding:"10px 14px",fontSize:12,color:"#eab308",marginBottom:8}}>[!] Could not load live data</div>}
      ...
      {tab==="escalations"&&(
        <div>
          ...
          <DragList items={escalations} onReorder={setEscalations} keyFn={e=>e.id} renderItem={(esc)=>(
      ...
      {tab==="pst"&&(
        <div>
          ...
          <DragList items={pstRoster} onReorder={setPstRoster} keyFn={m=>m.id} renderItem={(m)=>(
      ...
      {tab==="resources"&&isAdmin&&(
        <div>
          ...
          <DragList items={resources} onReorder={setResources} keyFn={r=>r.id} renderItem={(r)=>(
          ...
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            {["all","active","inactive"].map(f=>(
              <div key={f} onClick={()=>setRosterFilter(f)} style={{flex:1,padding:"9px 6px",borderRadius:10,cursor:"pointer",textAlign:"center",background:rosterFilter===f?"rgba(56,189,248,0.12)":"rgba(255,255,255,0.03)",border:"1px solid "+(rosterFilter===f?"rgba(56,189,248,0.3)":"rgba(255,255,255,0.07)"),fontSize:11,fontWeight:rosterFilter===f?800:600,color:rosterFilter===f?"#38bdf8":"#64748b"}}>
                ...
                <span style={{fontSize:10,marginLeft:4,color:rosterFilter===f?"#38bdf8":"#334155"}}>({f==="all"?roster.length:roster.filter(e=>e.status===f).length})</span>
              </div>
            ))}
          </div>
      ...
      {showConfirm&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,padding:20}}>
          <div style={{background:"#0c1929",border:"1px solid rgba(255,255,255,0.08)",borderRadius:18,padding:"28px 24px",maxWidth:320,width:"100%",textAlign:"center"}}>
            <div style={{fontSize:36,marginBottom:14}}>{showConfirm==="pst"?"✓":showConfirm==="critical"?"⚡":showConfirm==="notification"?"📢":showConfirm==="anon_submitted"?"🔒":showConfirm==="member_added"?"👥":"✓"}</div>
            ...
            <div style={{fontSize:12,color:"#475569",lineHeight:1.6,marginBottom:22}}>{showConfirm==="anon_submitted"?"Your anonymous report has been submitted to the PST team.":showConfirm==="roster_imported"?"Employee roster has been updated.":"Action completed successfully."}</div>
