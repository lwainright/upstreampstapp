...
function detectLevel(text) {
  ...
  const level3 = [
    "end it", "kill myself", "not worth living", "want to die", "suicide",
    "can't go on", "no point", "done with everything", "end my life",
    "hurt myself", "self harm", "can't take it anymore", "nothing left"
  ];
  const level2 = [
    "breaking down", "falling apart", "can't cope", "losing it",
    "not okay", "really struggling", "can't sleep", "nightmares",
    "flashback", "keeps coming back", "numb", "burned out",
    "drinking more", "drinking to cope", "can't stop thinking"
  ];
  const level1 = [
    "rough shift", "rough week", "not great", "stressed", "overwhelmed",
    "on edge", "irritable", "snapping", "withdrawn", "heavy", "hard lately",
    "struggling", "tired of it", "worn out"
  ];
  if (level3.some(w => t.includes(w))) return 3;
  if (level2.some(w => t.includes(w))) return 2;
  if (level1.some(w => t.includes(w))) return 1;
  return 0;
}
...
function detectSpiritual(text) {
  const t = text.toLowerCase();
  const words = [
    "pray", "prayer", "god", "faith", "church", "chaplain", "bible",
    "spiritual", "scripture", "pastor", "minister", "religious",
    "believe in god", "lord", "jesus", "allah", "higher power"
  ];
...
export default function AIChatScreen({ navigate, agency, userLanguage = "en", userState }) {
  ...
  const [aiName,         setAiName]         = useState(() => {
    try { return localStorage.getItem("upstream_ai_name") || "UPSTREAM AI"; } catch (e) { return "UPSTREAM AI"; }
  });
...
- If someone tests the app or asks something casual or technical, just answer normally.
...
Respond only with your reply. No labels, no formatting. Just speak.`;
  ...
  const send = async () => {
    ...
    try {
      ...
      setApiError(true);
      const fallbacks = ["I'm here — keep going.", "Yeah, I'm listening.", "Go ahead, I'm with you.", "I hear you. What else?"];
        ...
        {editingName && (
          <div style={{ display: "flex", gap: 8, alignItems: "center", padding: "8px 12px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10 }}>
            <input
              ...
              onKeyDown={e => {
                if (e.key === "Enter") { const n = nameInput.trim() || "UPSTREAM AI"; setAiName(n); try { localStorage.setItem("upstream_ai_name", n); } catch (err) {} setEditingName(false); }
            ...
            />
            <div onClick={() => { const n = nameInput.trim() || "UPSTREAM AI"; setAiName(n); try { localStorage.setItem("upstream_ai_name", n); } catch (err) {} setEditingName(false); }}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <div onClick={isListening ? stopVoice : startVoice}
              ...
              {isListening
                ? <><div style={{ display: "flex", gap: 3 }}>{[1, 2, 3, 4].map(i => <div key={i} style={{ width: 3, height: 8 + i * 2, background: "#f87171", borderRadius: 2 }} />)}</div>Stop</>
                : <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/></svg>Speak</>
              }
            </div>
            {input && (
              <div onClick={!isThinking ? send : undefined}
