...
const PLATFORM_FEATURES = [
  ...
  { key: "ai_chat",            label: "AI Peer Support",              sub: "Claude-powered anonymous AI chat",                       group: "Core" },
  { key: "safety_vault",       label: "Safety Vault",                 sub: "PIN-protected private safety area",                      group: "Core" },
  { key: "family_connect",     label: "Family Connect",               sub: "Cross-device private session chat",                      group: "Family" },
  { key: "family_codes",       label: "Family Codes",                 sub: "Responder generates codes for family members",           group: "Family" },
  { key: "kids_checkin",       label: "Kids Check-In",                sub: "Emoji check-in for under 12 family members",             group: "Family" },
  { key: "veterans_screen",    label: "Veterans Section",             sub: "Full veteran resources and education",                   group: "Populations" },
  { key: "telecom_screen",     label: "Telecommunications Section",   sub: "911 dispatch and comm center resources",                 group: "Populations" },
  { key: "human_services",     label: "Human Services Section",       sub: "DSS/CPS/APS worker wellness",                           group: "Populations" },
  { key: "retirees_screen",    label: "Retirees Section",             sub: "Retired FR and veteran resources",                      group: "Populations" },
  { key: "high_acuity",        label: "High Acuity Decompression",    sub: "Peds, fatality, removal, ICAC-adjacent",                group: "Tools" },
  { key: "medical_vault",      label: "Medical Wellness Journal",     sub: "Lab values, imaging, symptom tracker",                  group: "Tools" },
  { key: "hrv_screen",         label: "HRV Check",                    sub: "Camera-based heart rate variability",                   group: "Tools" },
  { key: "qr_poster",          label: "QR Poster Generator",          sub: "Print-ready PST posters",                               group: "Admin" },
  { key: "excel_csv",          label: "Excel → CSV Converter",        sub: "On-device file converter",                              group: "Admin" },
...
function PlatformTogglePanel() {
  ...
  const toggle = async (key) => {
    ...
    setToggles(updated);
    try {
      localStorage.setItem("upstream_platform_toggles", JSON.stringify(updated));
      // Save to Appwrite platform_settings
      await databases.upsertDocument?.(DB_ID, 'platform_settings', 'feature_toggles', { value: JSON.stringify(updated) }).catch(() => {});
      setSaved(true);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      {GROUPS.map(group => (
        <div key={group}>
          <div style={{ fontSize:10, fontWeight:800, color:"#475569", letterSpacing:"0.14em", textTransform:"uppercase", marginBottom:8 }}>{group}</div>
          <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
            {PLATFORM_FEATURES.filter(f => f.group === group).map(feature => {
              const isOn = toggles[feature.key] !== false; // default on unless explicitly off
              return (
                <div key={feature.key} style={{ display:"flex", alignItems:"center", gap:12, padding:"11px 14px", borderRadius:12, background:feature.disabled?"rgba(255,255,255,0.01)":"rgba(255,255,255,0.03)", border:`1px solid ${feature.disabled?"rgba(255,255,255,0.04)":"rgba(255,255,255,0.07)"}`, opacity:feature.disabled?0.5:1 }}>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:13, fontWeight:700, color:feature.disabled?"#334155":"#dde8f4" }}>{feature.label}</div>
                    <div style={{ fontSize:11, color:"#475569", marginTop:1 }}>{feature.sub}</div>
                  </div>
                  {!feature.disabled && (
                    <div onClick={() => toggle(feature.key)} style={{ width:44, height:26, borderRadius:13, background:isOn?"rgba(56,189,248,0.25)":"rgba(255,255,255,0.08)", border:`1.5px solid ${isOn?"rgba(56,189,248,0.5)":"rgba(255,255,255,0.12)"}`, cursor:"pointer", position:"relative", transition:"all 0.2s", flexShrink:0 }}>
                      <div style={{ position:"absolute", top:3, left:isOn?20:3, width:16, height:16, borderRadius:"50%", background:isOn?"#38bdf8":"#475569", transition:"left 0.2s" }}/>
                    </div>
                  )}
                  {feature.disabled && <div style={{ fontSize:10, color:"#334155", background:"rgba(255,255,255,0.04)", padding:"2px 8px", borderRadius:6 }}>FUTURE</div>}
                </div>
              );
            })}
          </div>
      ...
      <div style={{ background:"rgba(56,189,248,0.05)", border:"1px solid rgba(56,189,248,0.1)", borderRadius:10, padding:"10px 14px", fontSize:11, color:"#334155", lineHeight:1.6 }}>
        Toggles apply platform-wide. Agency admins can further restrict features for their agency in their admin panel. Changes take effect on next app load.
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      <div style={{ background:"rgba(167,139,250,0.06)", border:"1px solid rgba(167,139,250,0.15)", borderRadius:12, padding:"12px 14px", fontSize:12, color:"#94a3b8", lineHeight:1.7 }}>
        PST members can see aggregate data to help them prepare — not individual identities. Agency heads can override this per agency in their admin panel.
      </div>
      {(agencies || []).map(agency => {
        const current = settings[agency.code] || "basic";
        return (
          <div key={agency.$id} style={{ background:"rgba(255,255,255,0.025)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:14, padding:"14px 16px" }}>
            <div style={{ fontSize:13, fontWeight:700, color:"#dde8f4", marginBottom:10 }}>{agency.name} <span style={{ fontSize:10, color:"#475569" }}>({agency.code})</span></div>
            <div style={{ display:"flex", gap:6 }}>
              {PST_VISIBILITY_OPTIONS.map(opt => (
                <div key={opt.key} onClick={() => updateSetting(agency.code, opt.key)}
                  style={{ flex:1, padding:"8px 6px", borderRadius:9, cursor:"pointer", textAlign:"center", background:current===opt.key?"rgba(167,139,250,0.15)":"rgba(255,255,255,0.03)", border:`1.5px solid ${current===opt.key?"rgba(167,139,250,0.4)":"rgba(255,255,255,0.07)"}` }}>
                  <div style={{ fontSize:11, fontWeight:700, color:current===opt.key?"#a78bfa":"#64748b" }}>{opt.label}</div>
                </div>
              ))}
            </div>
            <div style={{ fontSize:10, color:"#334155", marginTop:6 }}>
              {PST_VISIBILITY_OPTIONS.find(o => o.key === current)?.sub}
            </div>
          </div>
        );
      })}
...
function CountyPanel({ agencies, onRefresh }) {
  const [counties, setCounties] = React.useState(() => {
    try { return JSON.parse(localStorage.getItem("upstream_counties") || "[]"); } catch(e) { return []; }
  });
  const [newCounty, setNewCounty] = React.useState({ name:"", code:"", adminEmail:"" });
  ...
  const addCounty = () => {
    ...
    setCounties(updated);
    try { localStorage.setItem("upstream_counties", JSON.stringify(updated)); } catch(e) {}
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      {counties.map(county => (
        <div key={county.id} style={{ background:"rgba(255,255,255,0.025)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:14, padding:"14px 16px" }}>
          <div style={{ fontSize:13, fontWeight:700, color:"#dde8f4", marginBottom:4 }}>{county.name} <span style={{ fontSize:10, color:"#38bdf8" }}>({county.code})</span></div>
          {county.adminEmail && <div style={{ fontSize:11, color:"#475569", marginBottom:10 }}>Admin: {county.adminEmail}</div>}
          <div style={{ fontSize:10, fontWeight:700, color:"#475569", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:6 }}>Agencies in this county</div>
          <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
            {(agencies || []).map(ag => {
              const assigned = agencyAssignments[ag.code] === county.code;
              return (
                <div key={ag.$id} onClick={() => assignAgency(ag.code, assigned ? "" : county.code)}
                  style={{ padding:"4px 10px", borderRadius:8, cursor:"pointer", fontSize:11, fontWeight:700, background:assigned?"rgba(56,189,248,0.15)":"rgba(255,255,255,0.04)", border:`1px solid ${assigned?"rgba(56,189,248,0.35)":"rgba(255,255,255,0.08)"}`, color:assigned?"#38bdf8":"#64748b" }}>
                  {ag.code}
                </div>
              );
            })}
      ...
      {!adding && (
        <div onClick={() => setAdding(true)} style={{ padding:"10px", borderRadius:10, cursor:"pointer", textAlign:"center", background:"rgba(56,189,248,0.06)", border:"1px solid rgba(56,189,248,0.15)", fontSize:12, fontWeight:700, color:"#38bdf8" }}>
          + Add County / Organization
        </div>
      )}
      {adding && (
        <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:14, padding:"14px", display:"flex", flexDirection:"column", gap:8 }}>
          <input value={newCounty.name} onChange={e => setNewCounty(p=>({...p,name:e.target.value}))} placeholder="Organization name (e.g. Wake County)" style={inputStyle}/>
          <input value={newCounty.code} onChange={e => setNewCounty(p=>({...p,code:e.target.value.toUpperCase()}))} placeholder="Code (e.g. WAKE)" style={inputStyle}/>
          <input value={newCounty.adminEmail} onChange={e => setNewCounty(p=>({...p,adminEmail:e.target.value}))} placeholder="Admin email (optional)" style={inputStyle}/>
          <div style={{ display:"flex", gap:8 }}>
            <div onClick={() => setAdding(false)} style={{ flex:1, padding:"9px", borderRadius:8, cursor:"pointer", textAlign:"center", background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", fontSize:11, fontWeight:700, color:"#64748b" }}>Cancel</div>
            <div onClick={addCounty} style={{ flex:2, padding:"9px", borderRadius:8, cursor:"pointer", textAlign:"center", background:"rgba(56,189,248,0.1)", border:"1px solid rgba(56,189,248,0.25)", fontSize:11, fontWeight:700, color:"#38bdf8" }}>Add</div>
          </div>
        </div>
      )}
      ...
      <div style={{ background:"rgba(167,139,250,0.05)", border:"1px solid rgba(167,139,250,0.15)", borderRadius:12, padding:"14px 16px", marginTop:8 }}>
        ...
        <div style={{ fontSize:12, color:"#64748b", lineHeight:1.75 }}>
          When active, agencies or divisions without a PST team can request a subcontract PST member from the Upstream network. Subcontract PST members would see only high acuity volume for their assigned agency — no individual data. This feature is captured and ready to build when the time is right.
        </div>
        <div style={{ fontSize:11, color:"#334155", marginTop:8 }}>Status: Captured · Not yet active · Toggle available in Feature Toggles when ready</div>
      </div>
    </div>
  );
}
...
export default function PlatformInlineContent({ navigate, onGhostLogin }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 5, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 14, padding: 5, overflowX: "auto", marginBottom: 12 }}>
        {["agencies", "analytics", "toggles", "pst-config", "county", "branding", "reset-queue", "access-log"].map(tk => (
          <div key={tk} onClick={() => setTab(tk)} style={{ flexShrink: 0, minWidth: 80, textAlign: "center", padding: "10px 12px", borderRadius: 10, background: tab === tk ? "rgba(234,179,8,0.15)" : "transparent", border: `1px solid ${tab === tk ? "rgba(234,179,8,0.3)" : "transparent"}`, cursor: "pointer", fontSize: 11, fontWeight: tab === tk ? 800 : 600, color: tab === tk ? "#eab308" : "#8099b0", whiteSpace: "nowrap" }}>
            {{ agencies: "Agencies", analytics: "Analytics", toggles: "Toggles", "pst-config": "PST Config", county: "County", branding: "Branding", "reset-queue": "Reset Queue", "access-log": "Access Log" }[tk]}
          </div>
        ))}
      </div>
      ...
      {tab === "toggles" && (
        <div>
          <div style={{ fontSize:10, fontWeight:800, letterSpacing:"0.16em", textTransform:"uppercase", color:"#64748b", marginBottom:8 }}>Platform Feature Toggles</div>
          <div style={{ fontSize:12, color:"#64748b", lineHeight:1.6, marginBottom:16 }}>
            Turn features on or off globally or per agency without touching code. Changes take effect immediately.
          </div>
          <PlatformTogglePanel/>
        </div>
      )}
      ...
      {tab === "pst-config" && (
        <div>
          <div style={{ fontSize:10, fontWeight:800, letterSpacing:"0.16em", textTransform:"uppercase", color:"#64748b", marginBottom:8 }}>PST Visibility Configuration</div>
          <div style={{ fontSize:12, color:"#64748b", lineHeight:1.6, marginBottom:16 }}>
            Control what PST members and supervisors can see per agency. Agency heads can also manage this in their admin panel.
          </div>
      ...
      {tab === "county" && (
        <div>
          <div style={{ fontSize:10, fontWeight:800, letterSpacing:"0.16em", textTransform:"uppercase", color:"#64748b", marginBottom:8 }}>County & Multi-Agency Management</div>
          <div style={{ fontSize:12, color:"#64748b", lineHeight:1.6, marginBottom:16 }}>
            Group agencies under a county or organization. County admins see all their agencies — agencies cannot see each other.
          </div>
