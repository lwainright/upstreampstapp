...
export default function HumanPSTScreen({ navigate, agency, logoSrc }) {
  ...
  const typeColors = {
    ...
    Admin:     { color: "#eab308", bg: "rgba(234,179,8,0.12)"   },
  };
  ...
  const filteredMembers = pstMembers.filter(m => {
    ...
    if (filter === "PST" || filter === "Chaplain" || filter === "Therapist" || filter === "Admin") return m.type === filter;
    return true;
  }).filter(m =>
    searchQuery === "" ||
  ...
  const inputStyle = {
    ...
    borderRadius: 12, padding: "12px 14px", fontSize: 13, fontFamily: "'DM Sans',sans-serif",
    ...
    return (
      <ScreenSingle wide={true} headerProps={{ onBack: () => navigate("home"), logoSrc }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16, paddingTop: 20 }}>
          ...
          <div style={{ fontSize: 13, color: "#64748b", textAlign: "center", lineHeight: 1.7 }}>Human PST access is available to members of participating agencies. Enter your agency code to connect with your peer support team.</div>
          <div onClick={() => navigate("agencycode")} style={{ width: "100%", padding: "14px", borderRadius: 12, textAlign: "center", cursor: "pointer", background: "rgba(56,189,248,0.12)", border: "1.5px solid rgba(56,189,248,0.3)", fontSize: 14, fontWeight: 700, color: "#38bdf8" }}>Enter Agency Code →</div>
  ...
  return (
    <ScreenSingle wide={true} headerProps={{ onBack: () => { if (step !== "panel") setStep("panel"); else navigate("home"); }, logoSrc, agencyName: agency?.name }}>
      ...
      {step === "panel" && (
        <>
          ...
          {/* Desktop 2-column: member list left, actions + tools right */}
          <div style={{ display: "grid", gridTemplateColumns: isWide ? "1fr 1fr" : "1fr", gap: isWide ? 24 : 0, alignItems: "start" }}>

            {/* LEFT — search + member list */}
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search by name or specialty..." style={{ ...inputStyle }}/>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {["all", "available", "PST", "Chaplain", "Therapist", "Admin"].map(f => (
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: available ? 12 : 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          ...
                          <div>
                            ...
                            <div style={{ fontSize: 12, fontWeight: 700, color: (typeColors[m.type] || typeColors.PST).color }}>{m.type} · {m.unit}</div>
            ...
            {/* RIGHT — actions, broadcast, wellness check */}
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {Notification.permission === 'default' && (
                <div style={{ background: "rgba(56,189,248,0.06)", border: "1px solid rgba(56,189,248,0.2)", borderRadius: 12, padding: "14px 16px" }}>
                  ...
                  <div style={{ fontSize: 12, color: "#64748b", lineHeight: 1.6, marginBottom: 10 }}>Get alerted when your PST team responds. No location tracking.</div>
                  <div onClick={async () => { await requestPushPermission(databases, 'user', agency?.code); }} style={{ padding: "10px", borderRadius: 10, cursor: "pointer", textAlign: "center", background: "rgba(56,189,248,0.12)", border: "1.5px solid rgba(56,189,248,0.3)", fontSize: 13, fontWeight: 700, color: "#38bdf8" }}>
                    Allow Notifications
                  </div>
                </div>
              )}
      ...
      {step === "contact" && (
        <>
          <div style={{ background: "rgba(167,139,250,0.06)", border: "1px solid rgba(167,139,250,0.18)", borderRadius: 14, padding: "14px 16px" }}>
            <div style={{ fontSize: 13, color: "#c4b5fd", fontWeight: 600, marginBottom: 4 }}>{requestedPST ? `Contacting ${requestedPST.name}` : "Request PST Support"}</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isWide ? "1fr 1fr 1fr" : "1fr 1fr", gap: 8, marginBottom: 4 }}>
          ...
          {method && method !== "chat" && (
            <>
              ...
              <div style={{ display: "flex", flexDirection: isWide ? "row" : "column", gap: 8 }}>
                {urgencyLevels.map(u => (
                  <div key={u.key} onClick={() => setUrgency(u.key)} style={{ flex: isWide ? 1 : "unset", background: urgency === u.key ? `rgba(${u.color === "#ef4444" ? "239,68,68" : u.color === "#eab308" ? "234,179,8" : "34,197,94"},0.1)` : "rgba(255,255,255,0.03)", border: `1.5px solid ${urgency === u.key ? u.color : "rgba(255,255,255,0.07)"}`, borderRadius: 12, padding: "12px 16px", cursor: "pointer", display: "flex", alignItems: "center", gap: 12 }}>
      ...
      {step === "confirm" && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16, paddingTop: 20 }}>
          ...
          <div style={{ fontSize: 13, color: "#64748b", textAlign: "center", lineHeight: 1.7 }}>A PST member will contact you — urgency: <span style={{ color: "#a78bfa", fontWeight: 700 }}>{urgencyLevels.find(u => u.key === urgency)?.label}</span>.<br/><br/>Your name and number were submitted to the PST team only.</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 200px)" }}>
          <div style={{ background: "rgba(167,139,250,0.07)", border: "1px solid rgba(167,139,250,0.18)", borderRadius: 12, padding: "10px 14px", marginBottom: 10, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <div onClick={() => setChatBlurred(b => !b)} style={{ fontSize: 10, fontWeight: 700, color: "#475569", cursor: "pointer", padding: "4px 8px", borderRadius: 6, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>{chatBlurred ? "Unblur" : "🛡 Protect"}</div>
          ...
          <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10, padding: "4px 0", filter: chatBlurred ? "blur(6px)" : "none", transition: "filter 0.3s" }}>
            {chatMessages.map((m, i) => (
              <div key={i} style={{ alignSelf: m.from === "user" ? "flex-end" : "flex-start", maxWidth: isWide ? "60%" : "82%" }}>
