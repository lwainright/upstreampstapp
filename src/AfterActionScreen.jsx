...
export default function AfterActionScreen({navigate,agency}){
  ...
  const defs=[
    {title:"Call Impact",subtitle:"Step 1 of 3",question:"How did this call affect you?",key:"impact",color:"#f97316"},
    {title:"What's Sticking",subtitle:"Step 2 of 3",question:"What part is staying with you?",key:"reaction",color:"#eab308"}
  ];
  ...
  const handleNext = () => {
    ...
    setStep(nextStep);
    // Track completion when they reach the reset step (step 2)
    if (nextStep === 2) {
      trackDebrief((agency&&agency.code), 'afteraction', 2);
    }
  };
  ...
  return(
    <ScreenSingle headerProps={{onBack:()=>navigate("tools"),agencyName:(agency&&agency.name)}}>
      <div style={{display:"flex",gap:6,alignItems:"center"}}>
        {[0,1,2].map(i=><div key={i} style={{height:6,flex:i===step?2:1,borderRadius:3,background:i<step?"#22c55e":i===step?"#f97316":"rgba(255,255,255,0.08)",transition:"all 0.3s"}}/>)}
        <span style={{fontSize:11,color:"#2d4a66",marginLeft:6}}>Under 60 sec</span>
      </div>
      {step<2?(<>
        ...
        <Btn color={defs[step].color} bg={`rgba(${step===0?"249,115,22":"234,179,8"},0.1)`} onClick={handleNext}>Next →</Btn>
      </>):(<>
